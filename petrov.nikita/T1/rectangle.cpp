...
petrov::point_t petrov::Rectangle::getFrameRect()
{
  ...
  frame_rect_.pos_ = { ((2 * p1_.x_ + p3_.x_ - p1_.x_) / 2), ((2 * p1_.y_ + p3_.y_ - p1_.y_) / 2) };
...
{
  p1_.x_ -= (frame_rect_.width_ / 2) * (k - 1);
  p1_.y_ -= (frame_rect_.height_ / 2) * (k - 1);
  std::clog << p1_.x_ << " " << p1_.y_ << "\n"; // Remove after debug
  p2_.x_ -= (frame_rect_.width_ / 2) * (k - 1);
  p2_.y_ += (frame_rect_.height_  / 2) * (k - 1);
  p3_.x_ += (frame_rect_.width_ / 2) * (k - 1);
  p3_.y_ += (frame_rect_.height_ / 2) * (k - 1);
  p4_.x_ += (frame_rect_.width_ / 2) * (k - 1);
  p4_.y_ -= (frame_rect_.height_ / 2) * (k - 1);
